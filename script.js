...
function play () {
        ...
        maxValue = (maxValue > 999) ? 999 : maxValue;
                ...
                if (maxValue<=minValue) {
                        answerField.innerText = `Вы что-то напутали, проверьте минимум, максимум и нажмите играть`; 
                        gameRun = false;  
                } else  {
...
function transformtoText () {
        ...
        if (answerNumber < 0) {
                answerNumbertoText = 'минус ' + answerNumbertoText;                 /*добавляем слово минус для отрицательных чисел*/
        }
...
document.getElementById('btnLess').addEventListener('click', function () {
        if (gameRun){
                ...
                answerNumber  = Math.ceil((minValue + maxValue) / 2);                  /*середина следующего интервала*/
                orderNumber++;                                                  
                orderNumberField.innerText = orderNumber;                               /*меняем номер вопроса в строке N*/
